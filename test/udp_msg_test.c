...
int main(int argc, char** argv) {
    ...
    int iter=0;
    char buf[80];
    struct sockaddr_in address, myaddress;//处理网络通信的地址
    ...
    address.sin_addr.s_addr=inet_addr("10.0.0.100");//这里不一样
    address.sin_port=htons(port);

    bzero(&myaddress,sizeof(myaddress));
    myaddress.sin_family=AF_INET;
    myaddress.sin_addr.s_addr=htonl(INADDR_ANY);//这里不一样
    myaddress.sin_port=htons(myport);

    //创建一个 UDP socket

    socket_descriptor=socket(AF_INET,SOCK_DGRAM,0);//IPV4  SOCK_DGRAM 数据报套接字（UDP协议）
    if(bind(socket_descriptor,(struct sockaddr *)&myaddress,sizeof(struct sockaddr))==-1){
        printf("Bind error...%d\r\n",errno);
